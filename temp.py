...
def update_page_value_by_title(db_path, title_value, page_value):
    ...
    cursor = conn.cursor()
    cursor.execute("UPDATE projects SET Status = ? WHERE Title = ?", (page_value, title_value))
    conn.commit()
    conn.close()
...
update_page_value_by_title(db_path,'Audio Converter VST', 'Complete') #first from second to
